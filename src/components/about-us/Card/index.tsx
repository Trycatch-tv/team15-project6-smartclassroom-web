...
export const CardComponent: React.FC<CardProps> = ({ image, name, position, gitHub, linkedIn }) => {
    const imagePath = `/profilePics/${image}`;
    return (
        <div>
            <Card sx={{ mt: 3, maxWidth:"250px", transition: 'box-shadow 0.3s ease-in-out', '&:hover': { boxShadow: '0px 0px 15px rgba(0,0,0,0.3)' }  }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'center'}} >
                    <IconButton onClick={() => window.open(linkedIn, '_blank')} style={{color:"#0077B5"}}>
                        {" "}
                        <LinkedInIcon />
                    </IconButton>
                    <IconButton onClick={() => window.open(gitHub, '_blank')} style={{color:"#620296"}}>
                        {" "}
                        <GitHubIcon />
                    </IconButton>
                </div>
